...
class fraction final {
   ...
   public:
	...
	fraction(const big_int &numerator) : _numerator(numerator), _denominator(1) {
	...
	fraction(T numerator, T denominator = 1) : _numerator(numerator), _denominator(denominator) {
		if (denominator == 0) {
			throw std::invalid_argument("Denominator cannot be zero");
		}
		optimise();
