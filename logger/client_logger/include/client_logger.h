...
private:
  ...
  class refcounted_stream final {
    static std::unordered_map<std::string, std::pair<size_t, std::ofstream>>
        _global_streams;
...
private:
  ...
  client_logger(
      const std::unordered_map<
          logger::severity,
          std::pair<std::forward_list<refcounted_stream>, bool>> &streams,
      std::string format);
...
public:
  [[nodiscard]] logger &log(const std::string &message,
