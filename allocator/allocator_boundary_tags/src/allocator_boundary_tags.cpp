...
allocator_boundary_tags::~allocator_boundary_tags() {
	...
	logger_instance->debug("start deleting alloc");
	if (!_trusted_memory) {
		logger_instance->error("can't dealloc");
		return;
	}
	...
	try {
		...
		size_t total_size =
		    ...
		    reinterpret_cast<size_t>(static_cast<char *>(_trusted_memory) + sizeof(logger *) +
		                             sizeof(memory_resource *) + sizeof(allocator_with_fit_mode::fit_mode));

		if (parent_allocator) {
			parent_allocator->deallocate(_trusted_memory, total_size);
			logger_instance->trace("mem dealloc using parent alloc");
		} else {
			::operator delete(_trusted_memory);
			logger_instance->trace("mem dealloc using delete");
		}

		_trusted_memory = nullptr;
	} catch (const std::exception &ex) {
		logger_instance->error("err deleting alloc " + std::string(ex.what()));
		_trusted_memory = nullptr;
	}

	logger_instance->trace("finished deleting alloc");
}
...
allocator_boundary_tags &allocator_boundary_tags::operator=(allocator_boundary_tags &&other) noexcept {
	...
	if (this != &other) {
		...
		std::cout << "mutex lock assign op" << std::endl;
		std::lock(lock_this, lock_other);
		if (_trusted_memory) {
			try {
				size_t total_size =
				    *reinterpret_cast<size_t *>(reinterpret_cast<unsigned char *>(_trusted_memory) + sizeof(logger *) +
				                                sizeof(memory_resource *) + sizeof(allocator_with_fit_mode::fit_mode));
				...
				log->error("err in assign op" + std::string(e.what()));
			}
		}

		_trusted_memory = other._trusted_memory;
		other._trusted_memory = nullptr;
		log->debug("moved resources");
	}

	trace_with_guard("finished assing op");
	return *this;
}

allocator_boundary_tags::allocator_boundary_tags(size_t space_size, std::pmr::memory_resource *parent_allocator,
                                                 logger *logger, allocator_with_fit_mode::fit_mode allocate_fit_mode) {
	logger->debug("alloc constructor start");
	if (space_size == 0) {
	...
	try {
		...
		logger->debug("alloc needs " + std::to_string(total_size) + " bytes");
		logger->information("aval " + std::to_string(space_size) + " bytes");
		...
		*reinterpret_cast<void **>(memory) = nullptr;
		...
		logger->error("failed init alloc");
		throw std::iostream ::failure("failed init alloc");
	}

	logger->debug("end init alloc");
}

[[nodiscard]] void *allocator_boundary_tags::do_allocate_sm(size_t size) {
	logger *logger = get_logger();
	logger->debug("alloc start");
	const size_t total_size = size + occupied_block_metadata_size;
	size_t allocator_size =
	    *reinterpret_cast<size_t *>(reinterpret_cast<char *>(_trusted_memory) + sizeof(class logger *) +
	                                sizeof(memory_resource *) + sizeof(allocator_with_fit_mode::fit_mode));
	if (total_size > allocator_size) {
		logger->error("ur size is too big to allocate");
		throw std::bad_alloc();
	}
	...
	switch (get_fit_mode()) {
		...
		case fit_mode::the_worst_fit:
			...
			break;
		default:
			logger->error("hz fit mode");
			throw std::invalid_argument("hz fit mode");
	}

	if (!allocated_memory) {
		logger->error("alloc crushed for size " + std::to_string(size));
		throw std::bad_alloc();
	}

	logger->debug("alloc end");
	return allocated_memory;
}

void allocator_boundary_tags::do_deallocate_sm(void *at) {
	logger *logger = get_logger();
	logger->debug("dealloc start");
...
inline void allocator_boundary_tags::set_fit_mode(allocator_with_fit_mode::fit_mode mode) {
	auto *memory = reinterpret_cast<unsigned char *>(_trusted_memory);
	auto *mode_ptr =
	    reinterpret_cast<allocator_with_fit_mode::fit_mode *>(memory + sizeof(logger *) + sizeof(memory_resource *));
	*mode_ptr = mode;
}

std::vector<allocator_test_utils::block_info> allocator_boundary_tags::get_blocks_info_inner() const {
	logger *logger = get_logger();
	logger->trace("start getting info");
	std::vector<allocator_test_utils::block_info> blocks_info;

	if (!_trusted_memory) {
		logger->error("trusted mem doesn't exist");
		return blocks_info;
	}
	...
	try {
		...
		while (current_block && current_block < heap_end) {
			...
			blocks_info.push_back(
			    {.block_size = block_size + occupied_block_metadata_size, .is_block_occupied = is_occupied});
...
std::vector<allocator_test_utils::block_info> allocator_boundary_tags::get_blocks_info() const {
	logger *logger = get_logger();
	logger->trace("Get_blocks_info start");
	std::lock_guard<std::mutex> guard(get_mutex());
	auto result = get_blocks_info_inner();

	logger->information("got " + std::to_string(result.size()) + " blocks");
	logger->trace("Get_blocks_info end");
...
inline std::string allocator_boundary_tags::get_typename() const noexcept { return "allocator_boundary_tags"; }
...
allocator_boundary_tags::boundary_iterator allocator_boundary_tags::end() const noexcept {
	size_t total_size =
	    *reinterpret_cast<size_t *>(reinterpret_cast<char *>(_trusted_memory) + sizeof(logger *) +
	                                sizeof(memory_resource *) + sizeof(allocator_with_fit_mode::fit_mode));
...
void *allocator_boundary_tags::boundary_iterator::get_ptr() const noexcept { return _occupied_ptr; }

std::pmr::memory_resource *allocator_boundary_tags::get_parent_resource() const noexcept {
	if (!_trusted_memory) return nullptr;
	return *reinterpret_cast<std::pmr::memory_resource **>(reinterpret_cast<unsigned char *>(_trusted_memory) +
	                                                       sizeof(logger *));
}

inline std::mutex &allocator_boundary_tags::get_mutex() const {
	if (!_trusted_memory) throw std::logic_error("no bitches(???");

	auto *ptr = reinterpret_cast<unsigned char *>(_trusted_memory) + sizeof(logger *) + sizeof(memory_resource *) +
...
void *allocator_boundary_tags::allocate_best_fit(size_t size) {
	...
	if (best_pos)
		return allocate_in_hole(reinterpret_cast<char *>(best_pos), size, first_block_ptr, best_prev, best_next,
		                        best_diff);
	return nullptr;
}
...
void *allocator_boundary_tags::allocate_worst_fit(size_t size) {
	...
	if (worst_pos)
		return allocate_in_hole(reinterpret_cast<char *>(worst_pos), size, first_block_ptr, worst_prev, worst_next,
		                        worst_size);
	return nullptr;
}
...
void *allocator_boundary_tags::allocate_in_hole(char *address, size_t size, void **first_block_ptr, void *prev_block,
                                                void *next_block, size_t size_free) {
	if (size_free - size - occupied_block_metadata_size < occupied_block_metadata_size) {
		size += size_free - size - occupied_block_metadata_size;
	}
	...
	if (next_block) {
		...
		if (current_end == next_block_start) {
			...
			*reinterpret_cast<void **>(current_block + sizeof(size_t)) = next_next;
			...
			if (next_next) {
				*reinterpret_cast<void **>(reinterpret_cast<char *>(next_next) + sizeof(size_t) + sizeof(void *)) =
				    current_block;
			}

			logger->debug("merged wth nxt block");
		}
	}
	...
	if (prev_block) {
		...
		if (prev_end == current_block) {
			...
			*reinterpret_cast<void **>(prev_block_start + sizeof(size_t)) = next_block1;
			...
			if (next_block1) {
				*reinterpret_cast<void **>(reinterpret_cast<char *>(next_block1) + sizeof(size_t) + sizeof(void *)) =
				    prev_block_start;
			}
			...
			logger->debug("merged wth prev block");
		}
	}
