...
template <typename T>
pp_allocator<T>::pp_allocator(std::pmr::memory_resource* mem) noexcept
    : _mem(mem == nullptr ? std::pmr::get_default_resource() : mem) {}
...
U* pp_allocator<T>::allocate_object(std::size_t n) {
	if ((std::numeric_limits<size_t>::max() / sizeof(U)) < n) throw std::bad_array_new_length();
