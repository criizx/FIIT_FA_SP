...
template <typename tkey, typename tvalue> struct test_data {
  ...
  test_data(size_t dep, tkey k, tvalue v, size_t h)
      : depth(dep), key(k), value(v), height(h) {}
};
...
bool infix_iterator_test(
  ...
  for (; vec_begin != vec_end; ++vec_begin) {
    ...
    if (it.depth() != item.depth || it->first != item.key ||
        it->second != item.value || it.get_height() != item.height) {
      return false;
    }
...
TEST(AVLTreePositiveTests, test1) {
  std::unique_ptr<logger> logger(
      create_logger(std::vector<std::pair<std::string, logger::severity>>{
          {"AVL_tree_tests_logs.txt", logger::severity::trace},
      }));

  logger->trace("AVLTreePositiveTests.test1 started");

  auto avl = std::make_unique<AVL_tree<int, std::string>>(
...
TEST(AVLTreePositiveTests, test2) {
  ...
  auto avl = std::make_unique<AVL_tree<int, int>>(std::less<int>(), nullptr,
                                                  logger.get());
  ...
  std::vector<test_data<int, int>> expected_result = {
      test_data<int, int>(0, 2, 12, 3), test_data<int, int>(1, 1, 5, 1),
      test_data<int, int>(1, 4, 45, 2), test_data<int, int>(2, 3, 67, 1),
...
TEST(AVLTreePositiveTests, test3) {
  ...
  auto avl = std::make_unique<AVL_tree<std::string, int>>(
...
TEST(AVLTreePositiveTests, test4) {
  ...
  auto avl1 = std::make_unique<AVL_tree<int, std::string>>(
      std::less<int>(), nullptr, logger.get());
