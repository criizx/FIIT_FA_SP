...
TEST(splayTreePositiveTests, test1) {
  std::unique_ptr<logger> logger(
      create_logger(std::vector<std::pair<std::string, logger::severity>>{
          {"splay_tree_tests_logs.txt", logger::severity::trace},
      }));

  logger->trace("splayTreePositiveTests.test1 started");

  auto splay = std::make_unique<splay_tree<int, std::string>>(
...
TEST(splayTreePositiveTests, test2) {
  ...
  auto splay = std::make_unique<splay_tree<int, int>>(std::less<int>(), nullptr,
                                                      logger.get());
  ...
  std::vector<test_data<int, int>> expected_result = {
      test_data<int, int>(0, 4, 45), test_data<int, int>(1, 3, 67),
      test_data<int, int>(2, 2, 12), test_data<int, int>(3, 1, 5),
...
TEST(splayTreePositiveTests, test3) {
  ...
  auto splay = std::make_unique<splay_tree<std::string, int>>(
...
TEST(splayTreePositiveTests, test4) {
  ...
  auto splay1 = std::make_unique<splay_tree<int, std::string>>(
      std::less<int>(), nullptr, logger.get());
...
TEST(splayTreePositiveTests, test10) {
  ...
  std::vector<typename splay_tree<int, std::string>::value_type> actual_result(
      b, e);

  std::vector<typename splay_tree<int, std::string>::value_type>
