...
TEST(binarySearchTreePositiveTests, noIteratorTest) {
  std::unique_ptr<logger> logger(
      create_logger(std::vector<std::pair<std::string, logger::severity>>{
          {"binary_search_tree_tests_logs.txt", logger::severity::trace}}));
  logger->trace("binarySearchTreePositiveTests.test1 started");

  auto al = std::make_unique<allocator_sorted_list>(10000);

  auto bst = std::make_unique<binary_search_tree<int, std::string>>(
      std::less<int>(), al.get(), logger.get());
  //    auto bst = new binary_search_tree<int, std::string>(key_comparer(),
  //    al.get(), logger.get());
...
TEST(binarySearchTreePositiveTests, test2) {
  ...
  auto bst = std::make_unique<binary_search_tree<int, int>>(
      std::less<int>(), pp_allocator<int>(), logger.get());
  ...
  std::vector<test_data<int, int>> expected_result = {
      test_data<int, int>(0, 1, 5), test_data<int, int>(1, 2, 12),
      test_data<int, int>(2, 15, 1), test_data<int, int>(3, 3, 67),
...
TEST(binarySearchTreePositiveTests, test3) {
  ...
  auto bst = std::make_unique<binary_search_tree<std::string, int>>(
...
TEST(binarySearchTreePositiveTests, test4) {
  ...
  auto bst1 = std::make_unique<binary_search_tree<int, std::string>>(
      std::less<int>(), nullptr, logger.get());
