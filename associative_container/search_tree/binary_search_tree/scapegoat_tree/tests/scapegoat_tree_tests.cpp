...
TEST(scapegoatTreePositiveTests, test1) {
  std::unique_ptr<logger> logger(
      create_logger(std::vector<std::pair<std::string, logger::severity>>{
          {"scapegoat_tree_tests_logs.txt", logger::severity::trace},
      }));

  logger->trace("scapegoatTreePositiveTests.test1 started");

  auto sg = std::make_unique<scapegoat_tree<int, std::string>>(
      std::less<int>(), nullptr, logger.get(), 0.7);
...
TEST(scapegoatTreePositiveTests, test2) {
  ...
  auto sg = std::make_unique<scapegoat_tree<int, int>>(
      std::less<int>(), nullptr, logger.get(), 0.5);
  ...
  std::vector<test_data<int, int>> expected_result = {
      test_data<int, int>(0, 3, 67), test_data<int, int>(1, 2, 12),
      test_data<int, int>(2, 1, 5), test_data<int, int>(1, 15, 1),
...
TEST(scapegoatTreePositiveTests, test3) {
  ...
  auto sg = std::make_unique<scapegoat_tree<std::string, int>>(
      std::less<std::string>(), nullptr, logger.get(), 0.9);
...
TEST(scapegoatTreePositiveTests, test4) {
  ...
  auto sg1 = std::make_unique<scapegoat_tree<int, std::string>>(
      std::less<int>(), nullptr, logger.get(), 0.65);
...
TEST(scapegoatTreePositiveTests, test10) {
  ...
  std::vector<typename scapegoat_tree<int, std::string>::value_type>
      actual_result(b, e);
